...
//! A factory of Rasters (GDALDatasets) from several data source types.
//! Does not take ownership of the pointer.
class GDALDatasetFactory {
public:
	//! Given a file path, returns a GDALDataset
	static GDALThreadSafeDataset *FromFile(const std::string &file_path,
	...
	//! Given a blob stream, returns a GDALDataset
	static GDALThreadSafeDataset *FromBlob(const char *blob, const uint64_t blob_size,
	                                       const std::vector<std::string> &allowed_drivers = std::vector<std::string>(),
	                                       const std::vector<std::string> &open_options = std::vector<std::string>());

	//! Writes a GDALDataset to a file path
	static bool WriteFile(GDALThreadSafeDataset *dataset, const std::string &file_path,
	                      const std::string &driver_name = "COG",
	                      const std::vector<std::string> &write_options = std::vector<std::string>());

	//! Writes a GDALDataset to a blob stream
	static const char *WriteBlob(GDALThreadSafeDataset *dataset, const std::string &driver_name, uint64_t &blob_size,
	                             const std::vector<std::string> &write_options = std::vector<std::string>());
